...
/* ── Middleware ── */
app.use('*', cors({ origin: ['https://novelnest.pages.dev','http://localhost:5173'], allowMethods:['GET','POST','PUT','DELETE','OPTIONS'] }))
app.use('*', compress())
app.use('*', etag())

/* ── Helpers ── */
const db = (c) => {
  const url = c.env?.SUPABASE_URL; const key = c.env?.SUPABASE_SERVICE_KEY
  if (!url || !key) return null
  return createClient(url, key, { auth: { persistSession: false } })
}
const ok  = (c, data, meta = {}) => c.json({ ok:true, data, ...meta })
const err = (c, msg, status=400) => c.json({ ok:false, message:msg }, status)
...
async function requireAdmin(c, next) {
  ...
  c.set('user', user); c.set('profile', profile)
  return next()
}
...
app.get('/novels', cache({ cacheName:'novels', cacheControl:'max-age=60,s-maxage=120' }), async (c) => {
  ...
  let q = supabase.from('novel_summary').select('*').range((page-1)*limit, page*limit-1)
...
app.get('/novels/search', async (c) => {
  ...
  const supabase = db(c)
  if (!supabase) return ok(c, MOCK_NOVELS.filter(n => n.title.toLowerCase().includes(q.toLowerCase())))
...
app.get('/novels/:id', cache({ cacheName:'novel', cacheControl:'max-age=120,s-maxage=300' }), async (c) => {
  const supabase = db(c)
  if (!supabase) return ok(c, MOCK_NOVELS.find(n=>n.id===c.req.param('id'))||MOCK_NOVELS[0])
  const { data, error } = await supabase.from('novel_summary').select('*').eq('id',c.req.param('id')).single()
...
app.post('/novels', requireAdmin, async (c) => {
  ...
  const { data, error } = await supabase.from('novels').insert({ ...body, author_id: c.get('user').id }).select().single()
...
app.put('/novels/:id', requireAdmin, async (c) => {
  ...
  const { data, error } = await supabase.from('novels').update({ ...body, updated_at: new Date() }).eq('id',c.req.param('id')).select().single()
  if (error) return err(c, error.message)
  return ok(c, data)
})
...
app.get('/novels/:id/chapters', cache({ cacheName:'chapters', cacheControl:'max-age=60' }), async (c) => {
  ...
  const { data, error } = await supabase.from('chapters')
    .select('id,chapter_num,title,word_count,created_at').eq('novel_id',c.req.param('id'))
    .eq('is_draft',false).order('chapter_num',{ascending:true})
  if (error) return err(c,error.message,500)
  return ok(c, data)
})
...
app.get('/chapters/latest', async (c) => {
  ...
  const { data } = await supabase.from('chapters')
    ...
    .eq('is_draft',false).order('created_at',{ascending:false}).limit(10)
  return ok(c, data || [])
})
...
app.get('/chapters/:id', cache({ cacheName:'chapter-content', cacheControl:'max-age=300,s-maxage=600' }), async (c) => {
  ...
  const { data, error } = await supabase.from('chapters')
    .select('id,novel_id,chapter_num,title,content,word_count,created_at').eq('id',c.req.param('id')).eq('is_draft',false).single()
  if (error) return err(c,'Not found',404)
...
app.post('/chapters', requireAdmin, async (c) => {
  ...
  const supabase = db(c)
  if (!supabase) return ok(c, { ...body, id: Date.now() })
  const { data, error } = await supabase.from('chapters').insert(body).select().single()
  if (error) return err(c, error.message)
  return ok(c, data)
})
...
app.put('/chapters/:id', requireAdmin, async (c) => {
  ...
  const supabase = db(c)
  if (!supabase) return ok(c, body)
  const { data, error } = await supabase.from('chapters').update({ ...body, updated_at:new Date() }).eq('id',c.req.param('id')).select().single()
  if (error) return err(c, error.message)
  return ok(c, data)
})
...
app.get('/analytics/summary', requireAdmin, async (c) => {
  ...
  return ok(c, [
    { icon:'👁', label:'Total Page Views', value: _fmt(views.count||0), delta:12, up:true  },
...
app.get('/analytics/pageviews', requireAdmin, async (c) => {
  ...
  const supabase = db(c)
  if (!supabase) return ok(c, { labels: MOCK_CHART_LABELS, values: MOCK_CHART_VALUES })
  const since = new Date(Date.now() - days * 86400000).toISOString()
  const { data } = await supabase.from('page_views').select('created_at').gte('created_at', since)
  // Group by day
  const byDay = {}
  ;(data || []).forEach(v => {
...
app.post('/auth/register', async (c) => {
  ...
  const supabase = db(c)
  if (!supabase) return ok(c, { user: { email }, mock: true })
  const { data, error } = await supabase.auth.admin.createUser({ email, password, email_confirm:false, user_metadata:{ username } })
  if (error) return err(c, error.message, 400)
  return ok(c, { user: data.user })
})

/* ── Health ── */
app.get('/health', (c) => c.json({ ok:true, ts:Date.now(), runtime:'cloudflare-pages-functions' }))
...
/* ── Fallback ── */
const MOCK_NOVELS = [
  { id:'1', title:'Sang Raja yang Dilupakan',    author_name:'Ariana Kusuma',  origin:'id', status:'ongoing',   avg_rating:4.8, total_views:890000, chapter_count:142 },
  { id:'2', title:'異世界転生の英雄譚',           author_name:'YamazakiRen',   origin:'jp', status:'ongoing',   avg_rating:4.9, total_views:2400000,chapter_count:387 },
]
const MOCK_ADS = [
  { id:'mock-1', slot:'banner',  active:true, title:'Bookwalker',  description:'Global novel store', cta:'Shop Now',  url:'#', icon:'🛒', impressions:45200, clicks:904  },
  { id:'mock-2', slot:'sidebar', active:true, title:'NovelAI',     description:'Write with AI',      cta:'Try Free',  url:'#', icon:'🤖', impressions:28100, clicks:1124 },
  { id:'mock-3', slot:'inline',  active:true, title:'Go Premium',  description:'Ad-free reading',    cta:'Upgrade',   url:'#', icon:'⭐', impressions:87400, clicks:1748 },
]
